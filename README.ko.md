...
Layer 3 — VM Integration        hack/test-vm-integration.sh
          seoy@100.123.80.48    실제 클러스터, conditions 회귀, SLI baseline
─────────────────────────────────────────────────────────────────────────────────
...
make vm-integration                         # Layer 3: 실제 클러스터 (SSH 필요)
make vm-integration ARGS=--update-baseline  # 현재 상태를 새 baseline으로 저장
```

kind 스모크 테스트는 docker 또는 podman이 필요하며 스크립트가 자동으로 감지합니다. rootless podman만 있는 환경에서는 user unit에 `systemd Delegate=yes` 설정이 필요합니다. CI(GitHub ubuntu-latest)는 Docker를 사용합니다.
...
| `vm-integration` | 3 | nightly + `workflow_dispatch` + main push | 실제 클러스터 conditions 회귀 |
